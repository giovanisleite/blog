...
Entretanto, o `userEvent.type` vai disparar eventos `keyDown`, `keyPress` e `keyUp`
para cada um dos caracteres. É muito mais próximo a interação real do usuário.
...
## Usar as variantes `query*` para qualquer coisa exceto checar coisas que não existem
...
ter uma função que você pode chamar que não estoura um erro se nenhum elemento é encontrado
(elas retornam `null` se nenhum elemento é encontrado). A _única_ razão para qual isso é útil é
...
`waitFor` é indicado para coisas que vão levar um quantidade de tempo indeterminada
entre a ação que você executou e a assertão que você está esperando se tornar verdade. Por conta disso,
o callback pode ser chamado (ou testado) um número indeterminado de vezes numa frequência indeterminada
(é chamado repetidas vezes com um intervalo e tabém sempre que o DOM sofre mutações). Então, isso significa que
...
Esse não é grande coisa na verdade, mas eu pensei em mencioná-lo e dar minha opinião sobre ele.
...
leitores do código que não é apenas uma query perdida depois de algum refactor
mas que eu estou explicitamente testando que aquilo existe.
...
falha, tentamos documentar as coisas corretamente. Mas isso pode ser realmente
difícil (especialmente conforme as APIs são alteradas/melhoradas/etc). Espero que isso tenha sido útil para
vocês. Nós realmente só queremos que você tenha mais sucesso na entrega de seu software
com confiança.
